...
(function (sandbox) {
    ...
    sampleArray2 = JSON.parse(fs.readFileSync(path.resolve(cwd + '/../jalangi2/src/js/runtime/sample_decay.json'), 'utf8'));
    sampleArrayLen = sampleArray2.length;
    sampleArray = new Uint16Array(sampleArrayLen);
    for (var i = 0; i < sampleArrayLen; i++) {
        sampleArray[i] = sampleArray2[i];
    }
    sampleArray2 = undefined;
    ...
    function invokeFun(iid, base, f, args, isConstructor, isMethod) {
        ...
        if (callAnalysis && sandbox.analysis && sandbox.analysis.invokeFun) {
